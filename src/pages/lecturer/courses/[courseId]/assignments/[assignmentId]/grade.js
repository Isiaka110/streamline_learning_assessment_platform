...
const AssignmentGradingPage = ({ initialAssignment, initialSubmissions, initialError }) => {
    ...
    const fetchGradingData = useCallback(async () => {
        ...
        } catch (_err) { // 🔑 2. FIX: Renamed 'err' to '_err'
            console.error('Fetch Grading Data Error:', _err);
    ...
    const handleSaveGrade = async (submissionId) => {
        ...
        try {
            ...
            console.error('Save Grade Error:', _err);
                                    ...
                                    {submission.filePath && (
                                        <div className="mt-4">
                                            ...
                                            {submission.filePath.match(/\.(jpeg|jpg|png|gif|webp)$/i) ? (
                                                <div className="relative w-full h-auto mt-2 rounded shadow overflow-hidden" style={{ maxHeight: '200px' }}>
                                                    {/* 🔑 3. FIX: Replaced <img> with <Image /> */}
                                                    <Image 
                                                        src={submission.filePath} 
                                                        alt="Submission File" 
                                                        fill 
                                                        sizes="(max-width: 768px) 100vw, 33vw"
                                                        style={{ objectFit: 'contain' }} // Use 'contain' to ensure image fits within the box
                                                        priority={false}
                                                    />
                                                </div>
                                            ) : (
